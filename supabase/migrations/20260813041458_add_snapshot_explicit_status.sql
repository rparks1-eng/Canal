...
comment on column public.snapshots.track_explicit is
  'Provider-supplied explicit-content status for the associated Snapshot track.';

create or replace function private.live_stage_tracks_are_safe(track_list jsonb)
returns boolean
language plpgsql
immutable
strict
...
begin
  ...
  for track in select item.value from jsonb_array_elements(track_list) as item(value)
  loop
    if jsonb_typeof(track) <> 'object'
      or not (track ?& array['id', 'title', 'artist', 'source'])
      or (track - array[
        'id', 'title', 'artist', 'source', 'spotifyUri', 'spotifyUrl',
        'durationMs', 'explicit', 'imageUrl', 'providerId', 'providerTrackId',
        'providerUrl', 'genreEvidence'
      ]::text[]) <> '{}'::jsonb
    then return false;
    end if;
    ...
    if (provider_id is null) <> (provider_track_id is null)
      ...
      or (provider_url is not null and (
    ...
    if track ? 'explicit' and jsonb_typeof(track -> 'explicit') <> 'boolean'
    then return false;
    end if;
...
revoke all on function private.live_stage_tracks_are_safe(jsonb)
from public, anon, authenticated, service_role;

grant execute on function private.live_stage_tracks_are_safe(jsonb)
